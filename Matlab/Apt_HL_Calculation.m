...
outfile = join(['results_Apparent_T50_', params.input_file], "_");

params.input_file   = input_file;
params.bin_size     = bin_size;
params.purity       = purity_of_SILAC_food;

%% READ INPUT WITHOUT RENAMING HEADERS 
try
    % Newer MATLAB (R2020a+)
...
SILAC_data_NaN    = cellfun(@(x) ~isa(x,'double'), SILAC_data); % possible 0x0 char reads
...
apparent_half_lives = zeros(size_Prot_data,1);
for i = 1:size_Prot_data
    Pdata_temp  = data.SILAC_data(:,i);
    non_NaN_idx = find(~isnan(Pdata_temp));
    Pdata_temp  = Pdata_temp(non_NaN_idx);
    rng default % For reproducibility
    ExpDegRate_temp        = fminsearch(@(b) norm(Pdata_temp - fun_deg(b, data.t(non_NaN_idx))), rand(1,1));
    apparent_half_lives(i) = ExpDegRate_temp;
end

apparent_half_lives = log(2)./apparent_half_lives;
...
SORT_SILAC = transpose(data.SILAC_data); %#ok<NASGU>
